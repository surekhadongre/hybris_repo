...
import com.nendrasys.facades.manufacturing.ManufacturingdetailsFacade;
import com.nendrasys.facades.populators.CustomFlagsPopulator;
...
public class ProductPageController extends AbstractPageController
{
	...
	@Resource(name = "manufacturingDetailsFacade")
	private ManufacturingdetailsFacade manufacturingdetailsFacade;

	@Resource(name = "manufacturingDetailsService")
			...
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		...
		final ProductData productData = this.productFacade.getProductForCodeAndOptions(productCode, extraOptions);

		final String redirection = checkRequestUrl(request, response, this.productDataUrlResolver.resolve(productData));
		...
		/*setting glag value to the model*/
		/*boolean flag=
...
*/
			...
			final HttpServletRequest request, final HttpServletResponse response) throws CMSItemNotFoundException
	{
		...
		{
			return REDIRECT_PREFIX + this.productDataUrlResolver.resolve(productData);
		}
	...
	{
		final String productCode = decodeWithScheme(encodedProductCode, UTF_8);
		final ProductData productData = this.productFacade.getProductForCodeAndOptions(productCode,
	...
	{
		final String productCode = decodeWithScheme(encodedProductCode, UTF_8);
		final ProductModel productModel = this.productService.getProductForCode(productCode);
		final ProductData productData = this.productFacade.getProductForCodeAndOptions(productCode,
				Arrays.asList(ProductOption.BASIC, ProductOption.PRICE, ProductOption.SUMMARY, ProductOption.DESCRIPTION,
						...
						ProductOption.VARIANT_FULL, ProductOption.DELIVERY_MODE_AVAILABILITY,ProductOption.FLAGS));
			...
			throws CMSItemNotFoundException
	{
		...
		final ProductData productData = this.productFacade.getProductForCodeAndOptions(productCode, null);
		if (result.hasErrors())
		{
			updatePageTitle(productCode, model);
        ...
        this.productFacade.postReview(productCode, review);
			...
			@PathVariable("numberOfReviews") final String numberOfReviews, final Model model, final HttpServletRequest request)
	{
		...
		{
			reviews = this.productFacade.getReviews(productCode);
		}
		else
		{
			...
			reviews = this.productFacade.getReviews(productCode, Integer.valueOf(reviewCount));
		}
	...
	protected void setUpReviewPage(final Model model, final String productCode) throws CMSItemNotFoundException
	{
		...
		setUpMetaData(model, metaKeywords, metaDescription);
		storeCmsPageInModel(model, getPageForProduct(productCode));
		model.addAttribute("product", this.productFacade.getProductForCodeAndOptions(productCode, Arrays.asList(ProductOption.BASIC)));
		updatePageTitle(productCode, model);
	}
			...
			final HttpServletRequest request, final HttpServletResponse response) throws CMSItemNotFoundException
	{
		...
		{
			final List<FutureStockData> futureStockList = this.futureStockFacade.getFutureAvailability(productCode);
	...
	public final Map<String, Object> productSkusFutureStock(final FutureStockForm form, final Model model)
	{
		...
		{
			final Map<String, List<FutureStockData>> futureStockData = this.futureStockFacade
			...
			final List<ProductOption> extraOptions) throws CMSItemNotFoundException
	{
		...
		final ProductData productData = this.productFacade.getProductForCodeAndOptions(productCode, options);

		sortVariantOptionData(productData);
		storeCmsPageInModel(model, getPageForProduct(productCode));
		populateProductData(productData, model);
		model.addAttribute(WebConstants.BREADCRUMBS_KEY, this.productBreadcrumbBuilder.getBreadcrumbs(productCode));
	...
	protected void populateProductData(final ProductData productData, final Model model)
	{
		...
		{
			final List<ConfigurationInfoData> configurations = this.productFacade.getConfiguratorSettingsForCode(productData.getCode());
	...
	protected void sortVariantOptionData(final ProductData productData)
	{
		if (CollectionUtils.isNotEmpty(productData.getBaseOptions()))
		{
			for (final BaseOptionData baseOptionData : productData.getBaseOptions())
			{
				...
				{
					Collections.sort(baseOptionData.getOptions(), this.variantSortStrategy);
				}
			}
		}
		...
		{
			Collections.sort(productData.getVariantOptions(), this.variantSortStrategy);
		}
	}
	...
	{
		final ProductModel productModel = this.productService.getProductForCode(productCode);
		return this.cmsPageService.getPageForProduct(productModel, getCmsPreviewService().getPagePreviewCriteria());
	}

	@ResponseBody
	@RequestMapping(value = "/manufacturingdetails",method = RequestMethod.GET,produces = MediaType.APPLICATION_JSON_VALUE)
	public ManufacturingDetailsData getManufacturingDetailsByCompanyName(@RequestParam final String companyName){
	   final ManufacturingDetailsData manufacturingDetailsData = this.manufacturingdetailsFacade.getManufacturingDetails(companyName);
	   return  manufacturingDetailsData;
    }

    @ResponseBody
    @RequestMapping(value = "/manufacturingDetailsByProduct",method = RequestMethod.GET,produces = MediaType.APPLICATION_JSON_VALUE)
    public ManufacturingDetailsData getManufacturingDetails(@RequestParam final String productCode){
		final ManufacturingDetailsData manufacturingDetailsData = this.manufacturingdetailsFacade.getManufacturingDetailsByProduct(productCode);
		return manufacturingDetailsData;
	}

	@ResponseBody
	@RequestMapping(value = "/allManufacturingDetailsRecord",method = RequestMethod.GET,produces = MediaType.APPLICATION_JSON_VALUE)
	public List<ManufacturingDetailsData> getAllManufacturingDetails(@RequestParam final int start, @RequestParam final int range){

		final List<ManufacturingDetailsData> manufacturingDetailsData= this.manufacturingdetailsFacade.getAllManufacturingDetails(start,range);
		System.out.println("in controller::size:"+manufacturingDetailsData.size());
		return manufacturingDetailsData;
	}
