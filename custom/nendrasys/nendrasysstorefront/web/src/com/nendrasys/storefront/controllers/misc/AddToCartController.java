...
public class AddToCartController extends AbstractController
{
			...
			@Valid final AddToCartForm form, final BindingResult bindingErrors)
	{
		...
		else
		{
			try
			{
				final CartModificationData cartModification = this.cartFacade.addToCart(code, qty);
				model.addAttribute(QUANTITY_ATTR, Long.valueOf(cartModification.getQuantityAdded()));
				model.addAttribute("entry", cartModification.getEntry());
				model.addAttribute("cartCode", cartModification.getCartCode());
				model.addAttribute("isQuote", this.cartFacade.getSessionCart().getQuoteData() != null ? Boolean.TRUE : Boolean.FALSE);
			...
			catch (final CommerceCartModificationException ex)
			{
				...
				model.addAttribute(QUANTITY_ATTR, Long.valueOf(0L));
				model.addAttribute(ERROR_MSG_TYPE,"basket.error.canBeSold");
			}
		...
		model.addAttribute("product", this.productFacade.getProductForCodeAndOptions(code, Arrays.asList(ProductOption.BASIC)));
	...
	public final String addGridToCart(@RequestBody final AddToCartOrderForm form, final Model model)
	{
		...
		{
			this.groupCartModificationListPopulator.populate(null, modificationDataList);
	...
	public String addEntryGroupToCart(final Model model, @Valid final AddToEntryGroupForm form, final BindingResult bindingErrors)
	{
		...
		try
		{
			...
			final CartModificationData cartModification = this.cartFacade.addToCart(addToCartParams);
			model.addAttribute(QUANTITY_ATTR, Long.valueOf(cartModification.getQuantityAdded()));
			model.addAttribute("entry", cartModification.getEntry());
			model.addAttribute("cartCode", cartModification.getCartCode());
		...
		model.addAttribute("product",
				this.productFacade.getProductForCodeAndOptions(form.getProductCode(), Arrays.asList(ProductOption.BASIC)));
			...
			final boolean isReducedQtyError)
	{
		...
		try
		{
			...
			final CartModificationData cartModificationData = this.cartFacade.addToCart(cartEntry.getProduct().getCode(), qty);
